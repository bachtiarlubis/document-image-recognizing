function varargout = cropSimalungun(varargin)
% CROPSIMALUNGUN MATLAB code for cropSimalungun.fig
%      CROPSIMALUNGUN, by itself, creates a new CROPSIMALUNGUN or raises the existing
%      singleton*.
%
%      H = CROPSIMALUNGUN returns the handle to a new CROPSIMALUNGUN or the handle to
%      the existing singleton*.
%
%      CROPSIMALUNGUN('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in CROPSIMALUNGUN.M with the given input arguments.
%
%      CROPSIMALUNGUN('Property','Value',...) creates a new CROPSIMALUNGUN or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before cropSimalungun_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to cropSimalungun_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help cropSimalungun

% Last Modified by GUIDE v2.5 04-Mar-2019 10:31:13

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @cropSimalungun_OpeningFcn, ...
                   'gui_OutputFcn',  @cropSimalungun_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before cropSimalungun is made visible.
function cropSimalungun_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to cropSimalungun (see VARARGIN)

% Choose default command line output for cropSimalungun
...
% UIWAIT makes cropSimalungun wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = cropSimalungun_OutputFcn(hObject, eventdata, handles) 
...
%     lineNumber = lineNumber+1;
    ...
    for m = 1 : lengthOfChar           
       ...
       letter=read_letter_simalungun(n1,num_letters); % read_letter_toba was changed to be read_letter_aksara
       word=[word letter];
       kata=[kata letter];
...
% saveDocName = sprintf('dok_%s.docx',fStyle);
% fullPathSave = fullfile(pwd, saveDocName);
document.SaveAs2([pwd '/dok_simalungun.docx']);
% document.SaveAs2(fullPathSave);
kata=[ ];
word.Quit();    
% winopen('dok_toba.docx')
winopen('dok_simalungun.docx')
