function varargout = cropKaro(varargin)
% CROPKARO MATLAB code for cropKaro.fig
%      CROPKARO, by itself, creates a new CROPKARO or raises the existing
%      singleton*.
%
%      H = CROPKARO returns the handle to a new CROPKARO or the handle to
%      the existing singleton*.
%
%      CROPKARO('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in CROPKARO.M with the given input arguments.
%
%      CROPKARO('Property','Value',...) creates a new CROPKARO or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before cropKaro_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to cropKaro_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help cropKaro

% Last Modified by GUIDE v2.5 04-Mar-2019 08:20:15

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @cropKaro_OpeningFcn, ...
                   'gui_OutputFcn',  @cropKaro_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before cropKaro is made visible.
function cropKaro_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to cropKaro (see VARARGIN)

% Choose default command line output for cropKaro
...
movegui('center');
% UIWAIT makes cropKaro wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = cropKaro_OutputFcn(hObject, eventdata, handles) 
...
%     lineNumber = lineNumber+1;
    ...
    for m = 1 : lengthOfChar           
       ...
       letter=read_letter_karo(n1,num_letters); % read_letter_toba was changed to be read_letter_aksara
       word=[word letter];
       kata=[kata letter];
