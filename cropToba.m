function varargout = cropToba(varargin)
% CROPTOBA MATLAB code for cropToba.fig
%      CROPTOBA, by itself, creates a new CROPTOBA or raises the existing
%      singleton*.
%
%      H = CROPTOBA returns the handle to a new CROPTOBA or the handle to
%      the existing singleton*.
%
%      CROPTOBA('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in CROPTOBA.M with the given input arguments.
%
%      CROPTOBA('Property','Value',...) creates a new CROPTOBA or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before cropToba_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to cropToba_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help cropToba

% Last Modified by GUIDE v2.5 04-Mar-2019 10:44:44

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @cropToba_OpeningFcn, ...
                   'gui_OutputFcn',  @cropToba_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before cropToba is made visible.
function cropToba_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to cropToba (see VARARGIN)

% Choose default command line output for cropToba
...
% UIWAIT makes cropToba wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = cropToba_OutputFcn(hObject, eventdata, handles) 
...
%     lineNumber = lineNumber+1;
    ...
    for m = 1 : lengthOfChar           
       ...
       letter=read_letter_toba(n1,num_letters); % read_letter_toba was changed to be read_letter_aksara
       word=[word letter];
       kata=[kata letter];
...
% saveDocName = sprintf('dok_%s.docx',fStyle);
% fullPathSave = fullfile(pwd, saveDocName);
document.SaveAs2([pwd '/dok_toba.docx']);
% document.SaveAs2(fullPathSave);
kata=[ ];
word.Quit();    
% winopen('dok_toba.docx')
